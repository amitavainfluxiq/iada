...
/* App Module */
var moduleappvar = angular.module('moduleapp', ['common_module_app','ui.router','angularValidator','ngCookies','ui.bootstrap','ngFileUpload','ui.tinymce','youtube-embed','angularLazyImg','bootstrapLightbox']);

moduleappvar.run(['$rootScope', '$state','contentservice','$uibModal','$log',function($rootScope, $state,contentservice,$uibModal,$log){
...
moduleappvar.service('contentservice', function($http, $log, $q) {
    var d;
...
moduleappvar.filter('startFrom', function () {
...
moduleappvar.filter('limitHtml', ['$sce',function($sce) {
...
moduleappvar.filter('limitText', ['$sce',function($sce) {
    return function(text, limit) {
...
moduleappvar.filter('newline', function($sce) {
...
moduleappvar.directive('loadedImg', function(){
    return {
        restrict: 'E',
        scope: {
            isrc: '=',
...
moduleappvar.directive('content',['$compile','$sce','$state','$rootScope', function($compile,$sce,$state,$rootScope) {
    var directive = {};
...
moduleappvar.filter("sanitize123", ['$sce', function($sce) {
...
moduleappvar.filter("sanitizelimit", ['$sce', function($sce) {
    return function(htmlCode){
        //console.log(htmlCode);
        //console.log('santize');
...
moduleappvar.config(function($stateProvider, $urlRouterProvider,$locationProvider) {
    $urlRouterProvider
        .otherwise("/home");
...
moduleappvar.controller('addcontent', function($compile,$scope,$state,$http,$cookieStore,$rootScope,Upload,$sce,$stateParams,$window) {
...
moduleappvar.controller('contentlist', function($scope,$state,$http,$cookieStore,$rootScope,$uibModal) {
...
moduleappvar.controller('editcontent', function(contentservice,$compile,$scope,$state,$http,$cookieStore,$rootScope,Upload,$sce,$stateParams,$uibModalInstance,items) {
...
moduleappvar.controller('ModalInstanceCtrl', function ($state,$cookieStore,$http,$rootScope,Upload,$uibModal,$timeout,$scope, $uibModalInstance, items) {
